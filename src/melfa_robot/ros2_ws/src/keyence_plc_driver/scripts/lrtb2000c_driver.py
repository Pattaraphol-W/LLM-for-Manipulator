...
class PLCProtocol(Node):
    ...
    def __init__(self):
        ...
        for device in self._read_devices:
            ...
            self._publishers[device.strip()] = self.create_publisher(Float32, topic_name, 10)
        ...
        # Timer for main read loop at 10Hz
        self._timer = self.create_timer(1.0 / 10.0, self._read_loop)
    ...
    def _read_loop(self):
        ...
        for device in self._read_devices:
            ...
            if device in self._publishers:
                self._publishers[device].publish(Float32(data=parsed_value))
                self.get_logger().info(f"Published {parsed_value} for {device}")
            else:
