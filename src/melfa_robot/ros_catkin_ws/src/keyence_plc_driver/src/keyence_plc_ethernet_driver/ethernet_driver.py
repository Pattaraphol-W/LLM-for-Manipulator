...
class PLCSocket:
    ...
    def connect(self):
        ...
        try:
            ...
            rospy.loginfo(f"PLCSocket: Successfully connected to {self.host}:{self.port}.")
            return True
        except Exception as e:
            rospy.logerr(f"PLCSocket: Failed to connect - {e}")
            self._connected = False
            return False
    ...
    def send_receive(self, command_bytes):
        ...
        if not self._connected:
            rospy.logwarn("PLCSocket: Not connected.")
            return None
        try:
            ...
            rospy.logwarn("PLCSocket: Socket operation timed out.")
            return None
        except socket.error as e:
            rospy.logerr(f"PLCSocket: Socket error during send/recv - {e}")
            self._connected = False
            return None
    ...
    def disconnect(self):
        ...
        if self._client:
            self._client.close()
        self._connected = False
        rospy.loginfo("PLCSocket: Disconnected.")
