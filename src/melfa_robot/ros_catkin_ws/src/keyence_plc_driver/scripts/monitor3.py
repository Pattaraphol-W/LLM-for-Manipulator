...
class CombinedMonitorGUI:
    def __init__(self, root):
        ...
        self.stick_update_interval = 0.1  # Update stick control every 100ms (10Hz max)
        ...
        # Active MR indicator
        tk.Label(sticks_frame, text="Active MR:", font=("Arial", 12, "bold"), 
                bg='#2c3e50', fg='#3498db').grid(row=2, column=0, padx=10, pady=8, sticky="w")
        self.active_mr_var = tk.StringVar(value="MR406")
        tk.Label(sticks_frame, textvariable=self.active_mr_var, font=("Arial", 16, "bold"), 
                bg='#2c3e50', fg='#2ecc71', width=8).grid(row=2, column=1, padx=10, pady=8)
        ...
        # Only the MR registers used in button mappings and stick control
        mr_addresses = ['MR206', 'MR207', 'MR208', 'MR209', 'MR400', 'MR401', 'MR402', 
    ...
    def _handle_stick_control(self, left_y, right_x):
        """
        Handle stick-based control for MR400-MR406.
        Only writes when the active MR changes to reduce command flooding.
        """
        active_mr = None
        
        # Determine which MR should be active based on stick positions
        if left_y > 0.05:
            if abs(right_x) < 0.2:  # right_x = 0
                active_mr = 'MR400'
            elif right_x > 0.2:
                active_mr = 'MR402'
            elif right_x < -0.2:
            ...
            if abs(right_x) < 0.2:  # right_x = 0
                ...
                active_mr = 'MR404'
        elif abs(left_y) < 0.2 and abs(right_x) < 0.2:  # Both sticks centered
            active_mr = 'MR406'
        
        # Only update if the active MR has changed
        if active_mr != self.last_active_mr:
            rospy.loginfo(f"Stick control: {self.last_active_mr} -> {active_mr}")
            
            # Turn off the previously active MR
            if self.last_active_mr is not None:
                self._publish_write_command(self.last_active_mr, 0)
                rospy.sleep(0.05)  # Small delay between commands
            
            # Turn on the new active MR
            if active_mr is not None:
                self._publish_write_command(active_mr, 1)
            
            # Update the last active MR
            self.last_active_mr = active_mr
            self.active_mr_var.set(active_mr if active_mr else "None")
