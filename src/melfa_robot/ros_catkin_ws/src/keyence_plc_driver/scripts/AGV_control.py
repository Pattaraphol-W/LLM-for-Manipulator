...
class AGVTeachingControl:
    def __init__(self, root):
        ...
        rospy.loginfo("AGV Teaching Control started")
    ...
    def setup_ros(self):
        rospy.init_node('agv_teaching_control', anonymous=True)
    ...
    def _toggle_plc(self, address):
        ...
        rospy.loginfo(f"{address} toggled: {current_state} -> {new_state}")
        ...
        if active_mr != self.last_active_mr:
            ...
            if self.is_recording and self.current_direction and self.direction_start_time:
                ...
                if duration > 0.1:
                    ...
                    rospy.loginfo(f"Recorded: {self._get_direction_name(self.current_direction)} for {duration:.2f}s")
        ...
        try:
            while self.is_playing and not rospy.is_shutdown():
                for i, direction in enumerate(self.taught_directions):
                    ...
                    # Activate direction
                    rospy.loginfo(f"Playing: {direction['direction']} for {duration:.2f}s")
