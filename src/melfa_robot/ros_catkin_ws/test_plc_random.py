...
NUM_RANDOM_COMMANDS = 20000

for i in range(NUM_RANDOM_COMMANDS):
    # Random command body
    cmd_len = random.randint(3, 15)
    # Prioritize common Keyence command prefixes for readability and relevance
    cmd_prefix = random.choice(['RD ', 'WR ', 'DM ', 'R ', 'W '])
    cmd_body = cmd_prefix + ''.join(random.choice(string.ascii_uppercase + string.digits + ' .') for _ in range(cmd_len - len(cmd_prefix)))

    # Random parameters
    unit_num = random.choice([0, 1, 10, None])
    station_num = random.choice([0, 1, None]) if unit_num is not None else None
    ...
    pad_body = random.choice([None, 5, 10, 15])
    ...
    # Ensure command_body_str is always used for this Keyence-focused random test
    raw_cmd_body = None
    ...
    try:
        ...
        # print(f"Skipping command generation due to error: {e}") # Commented out to reduce noise
        continue
...
if __name__ == "__main__":
    print("--- Testing Keyence-style Random Commands (Fixed STX=0x02) ---")
    for command_bytes, command_description in COMMANDS_TO_TEST:
        ...
        if raw_data and raw_data != b'\x82P': # Stop if response is not 8250
            ...
            # Attempt to decode the response if it's not the error code
            print("\n--- Decoding Attempts for Success Response ---")
            try:
        ...
        if raw_data:
            print("\n--- Decoding Attempts for Error Response ---")
            ...
            try:
                ...
                print(f"Shift-JIS: <Decoding failed - raw bytes not Shift-JIS>")
            print("------------------------------------------")
        # else: # Commented out to reduce log spam when no data is received
